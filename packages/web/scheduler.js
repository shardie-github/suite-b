...
function write(j){ fs.mkdirSync(ROOT,{recursive:true}); fs.writeFileSync(SFILE, JSON.stringify(j,null,2)); }
...
export function del(id){ const j=read(); j.jobs = (j.jobs||[]).filter(x=>x.id!==id); write(j); return {ok:true}; }

function saveCSV(tenant, rows){
  const dir = path.join(ROOT,"exports",tenant); fs.mkdirSync(dir,{recursive:true});
...
export function runNow(id){ const j=list().find(x=>x.id===id); if(!j) return {ok:false, error:"not_found"}; runJob(j); return {ok:true}; }
