...
export function setFlags(obj){ fs.mkdirSync(ROOT,{recursive:true}); fs.writeFileSync(FLAGS_FILE, JSON.stringify(obj,null,2)); }
...
export function importRows(tenant, rows=[]){
  ...
  for(const r of rows){
    ...
    obj.rows.push({ id:String(r.id), date:String(r.date), type:r.type||"Other", status:r.status||"New", email:r.email||"" });
    added++;
  }
